...
class StayUnitController(
	...
	fun getStayUnitsByStayId(@PathVariable stayId: Long): ResponseEntity<List<StayUnitResponse>> {
		val units = stayUnitService.getStayUnitsByStayId(stayId)
		return ResponseEntity.ok(units)
	}

	@GetMapping("/{id}")
	fun getStayUnitById(@PathVariable id: Long): ResponseEntity<StayUnitResponse> {
